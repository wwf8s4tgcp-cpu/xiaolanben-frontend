        ...
        <div class="user-content" :class="{ 'content-hidden': !avatarLoaded }">
            ...
            <div class="user-info">
                <div class="user-main">
                    ...
                    <div class="user-id">小蓝本号：{{ user.userId }}</div>
...
const isCurrentUser = computed(() => {
    ...
    const currentUserId = userStore.userInfo.user_id // 当前用户的小蓝本号
    const userId = props.user.user_id || props.user.userId // 用户的小蓝本号
...
const userHoverConfig = computed(() => ({
    ...
    onFollow: (userInfo) => {
        const userId = props.user.user_id || props.user.userId
        handleFollow(userId) // 使用小蓝本号进行关注操作
    },
    onUnfollow: (userInfo) => {
        const userId = props.user.user_id || props.user.userId
        handleUnfollow(userId) // 使用小蓝本号进行取消关注操作
    },
    delay: 500
}))
</script>
